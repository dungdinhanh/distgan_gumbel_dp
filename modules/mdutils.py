...
def load_checkpoint(checkpoint_dir, session, var_list=None):
    ...
    print('Checkpoint state: {}'.format(ckpt))
...
# select best choice of encoder to encode the feature
def gather_by_index(x, idx):
    idx_list  = tf.cast(tf.range(tf.shape(x)[1]),dtype=tf.int64)
    idx_merge = tf.transpose(tf.stack([idx, idx_list], axis=0))
    x_out = tf.gather_nd(x, idx_merge)
    return x_out, idx_merge
